...
class Polynomial(object):
    ...
    def __divmod__(self, other):
        zero = copy.deepcopy(self.coefficients[0] - self.coefficients[0])
        remainder = copy.deepcopy(self)
        p_zero = Polynomial([zero])
        one = other.coefficients[0]/other.coefficients[0]
        if other==Polynomial([one]):
            return (self, Polynomial([zero]))
        x = Polynomial([one, zero])
        quotient = Polynomial([zero])
        while remainder != p_zero and remainder.degree()>=other.degree():
            r_lead = remainder.coefficients[0]
            o_lead = other.coefficients[0]
